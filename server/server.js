...
app.get("/restart_server", (req, res) => {
	if (req.query.code !== "IwOsj8fK7jLt4EeyXPdiow==")
	...
	res.send(
		'<CENTER><h1 style="color: yellow">SERVER RESTARTING...</h1></CENTER>'
	);
});
...
app.get("/game/question/:id", (req, res) => {
	res.render("game/question", {
		question: questions[req.params.id]
			? JSON.parse(JSON.stringify(questions[req.params.id]))
			: undefined,
...
app.get(
	...
	(req, res) => {
		if (adminSocket !== "")
			return res
				.status(400)
				.send("<CENTER><h1>NOT AUTHORIZED</h1></CENTER>");
